...
void setup() {
    ...
    // Attach once with calibrated pulse range (min, max)
    ...
    if (wifiConnected) {
        ...
        if (isRotating) {
            if (currentMillis - lastRotationTime >= rotationTime) {
                ...
                // Pulso para girar en sentido invertido (anteriormente 1700)
                // Ajusta este valor si el servo necesita calibración (ej. 1400-1200)
                servo.writeMicroseconds(1300); // invertido
        ...
        if ((millis() - lastDebounceTime) > debounceDelay) {
            if (reading != buttonState) {
                ...
                if (buttonState == LOW && !isRotating) {
                    ...
                    // Pulso inicial para iniciar rotación en sentido invertido
